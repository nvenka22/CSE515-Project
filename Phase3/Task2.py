...
caltech101 = Caltech101(str(dirname) + "/caltech101",download=True)
...
option = st.selectbox(
        "Select number of clusters required from DBSCAN",
        ("5", "10"),
...
if st.button("Run", type="primary"):
    with st.spinner('Calculating...'):
        with st.container():
            if option == '5':
                model_dbscan(caltech101, feature_collection, odd_feature_collection,  data, 5)
            else:
                model_dbscan(caltech101, feature_collection, odd_feature_collection, data, 10)