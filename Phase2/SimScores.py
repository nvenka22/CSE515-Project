...
@jit(target_backend='cuda',nopython = False,fastmath = True, parallel = True) 
...
@jit(target_backend='cuda',nopython = False,fastmath = True, parallel = True) 
def call_similarity_calculator(startindex,endindex,dataset):

    for idx in range(startindex,endindex,2):
        client = get_client()
        dbName = "CSE515-MWD-ProjectPhase2"
        odd_feature_collection = get_collection(client,dbName,'image_features_odd')
        feature_collection = get_collection(client,dbName,'image_features')
        similarity_collection = get_collection(client,dbName,'image_similarities')
...
if __name__=="__main__": 
    ...
    processes = [Process(target = call_similarity_calculator,args = (idx,idx+250,caltech101)) for idx in range(0,2667,250)]
